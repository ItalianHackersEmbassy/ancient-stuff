...
Impact: local users can get root access
Vulnerable to this exploit: Irix 5.3 Irix64 6.2 Irix 6.3
Tested on: R3000 Indigo (5.3), R4400 Indy (5.3), R5000 O2 (6.3), R8000
           PChallenge (Irix64 6.2)
Temporary fix: chmod u-s /bin/login

Note this will break login, although it should still work when called
from telnetd, getty etc

Compile the exploit with the '-n32' flag if compiling under 6.x

When run, you will be prompted for a password. Just press enter at this
point and you will handed a root shell.
